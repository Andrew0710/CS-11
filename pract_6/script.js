...
const setupFilters = (data) => {
    ...
    const ratingRadios = document.querySelectorAll('input[name="rating-selector"]');
    ...
    document.querySelectorAll(
        ...
        checkboxInput.addEventListener(
            ...
            () => {
                ...
                if (selectedRating) {
                    ratingValue = parseInt(selectedRating.value);
                } 
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (savedCart) {
        ...
        parsedCart.forEach(el => cart.push(el))
    ...
    data.forEach(el => {
        const cartItem = cart.find(c => c.id === el.id)
        
        if (cartItem) {
            
            el.availableCount = el.availableCount - cartItem.count
        }
